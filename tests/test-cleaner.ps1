...
try {
    ...
    # --- Firefox 缓存：只清 cache 子目录，绝不碰 profile 数据（书签/密码/历史） ---
    Section "6. Firefox 缓存清理只碰 cache2，绝不删 profile 数据"
    $ffBase = Join-Path $sandbox 'ff\Profiles'
    $prof   = Join-Path $ffBase 'abcd.default-release'
    $cache2 = Join-Path $prof 'cache2'
    New-Item -ItemType Directory -Force -Path $cache2 | Out-Null
    [System.IO.File]::WriteAllBytes((Join-Path $cache2 'entry1'), (New-Object byte[] (256KB)))
    # 关键用户数据：书签库 / 密码 / 偏好——必须一根汗毛都不能动
    Set-Content -Path (Join-Path $prof 'places.sqlite') -Value 'bookmarks' -Encoding UTF8
    Set-Content -Path (Join-Path $prof 'logins.json')   -Value 'passwords' -Encoding UTF8
    Set-Content -Path (Join-Path $prof 'prefs.js')      -Value 'prefs'     -Encoding UTF8

    $ffSize = Get-FirefoxCacheSize $ffBase
    Check "Firefox 缓存体积算得到 (>=256KB)" ($ffSize.bytes -ge 256KB)
    $ffFreed = Clear-FirefoxCache $ffBase
    Check "清理释放 >=256KB"          ($ffFreed -ge 256KB)
    Check "cache2 已清空"             ($null -eq (Get-ChildItem -LiteralPath $cache2 -Force -ErrorAction SilentlyContinue))
    Check "书签 places.sqlite 保留"   (Test-Path -LiteralPath (Join-Path $prof 'places.sqlite'))
    Check "密码 logins.json 保留"     (Test-Path -LiteralPath (Join-Path $prof 'logins.json'))
    Check "偏好 prefs.js 保留"        (Test-Path -LiteralPath (Join-Path $prof 'prefs.js'))
    Check "profile 目录本身保留"      (Test-Path -LiteralPath $prof)
}
finally {
    Section "6. 清理沙盒"
