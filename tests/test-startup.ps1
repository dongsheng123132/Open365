...
try {
    ...
    Section "6. 开机项智能建议（本地名录，不联网、不上传）"
    # 端到端：list 的 JSON 里每项都带 advice 字段
    $advOfTest = ($data.items | Where-Object { $_.id -eq $testId }).advice
    Check "list 输出带 advice 字段" ($null -ne $advOfTest)
    # 单元：dot-source 引擎直接验分类（保守：要害保留、大厂常驻可关、未知交用户）
    . $engine help | Out-Null
    Check "Get-StartupAdvice 已加载"     ($null -ne (Get-Command Get-StartupAdvice -ErrorAction SilentlyContinue))
    Check "QQ → optional(可关)"           ((Get-StartupAdvice 'QQ' 'C:\qq.exe').level -eq 'optional')
    Check "Realtek 目录名 → keep(保留)"   ((Get-StartupAdvice 'RtkAud' 'C:\Program Files\Realtek\Audio\RtkAud.exe').level -eq 'keep')
    Check "Windows Defender → keep(保留)" ((Get-StartupAdvice 'Windows Defender' 'msmpeng.exe').level -eq 'keep')
    Check "未知项 → unknown(交用户)"      ((Get-StartupAdvice 'ZzzFooBar' 'C:\zzz.exe').level -eq 'unknown')
    # 回归：ProgramData 路径里的 'amd' 噪声不得误判 keep（Tailscale 铁证）
    Check "ProgramData 路径不误判 keep"   ((Get-StartupAdvice 'Tailscale' 'C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\Tailscale.lnk').level -eq 'unknown')
}
finally {
    Section "5. 清理测试项"
