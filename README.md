...
| 📦 **软件搬家** | 微信/QQ/钉钉/企业微信数据目录**整体搬到别的盘**（C 盘瘦身），原位置留目录联接、软件无感读写，**可一键还原** | 数据搬家、C盘瘦身 |
| 🔔 **检查更新** | 你点了才查有没有新版；**只发一个空 GET、不带机器码、不自动下载、不静默替换文件** | 软件升级 |
...
- **📦 软件搬家**：列出微信/QQ/钉钉/企业微信数据目录（位置/大小/状态），填目标盘一键搬家；每次搬家写记录，随时还原。

左下角版本号 / 托盘「关于 Open365」里有 **🔔 检查更新**——全程序唯一会主动出网的按钮，
点了才查，查完只告诉你有没有新版和去哪下，**不后台下载、不静默替换文件**（见下方「联网说明」）。
...
# 软件搬家（微信/QQ/钉钉/企业微信数据目录迁盘，可一键还原）
open365 relocate list             # 列出可搬家的数据目录（位置/大小/状态，只读）
open365 relocate status           # 看搬家记录（哪些搬过、状态如何，只读）
open365 relocate move -App qq -Target "D:\Open365搬家"   # 搬家（会交互确认，加 -Yes 跳过）
open365 relocate restore -App qq  # 还原回 C 盘原位置（会交互确认，加 -Yes 跳过）

# 检查更新（唯一会出网的命令，只查不装）
open365 update                    # 查有没有新版
open365 update sources            # 看它会去问哪个地址
open365 update check -Url https://你的镜像/version.json   # 自建/内网源（指定后不再回落 github）
```
...
- **透明**：核心是纯 PowerShell + bat；图形外壳是 C# WinForms（源码在 `gui/`，用系统自带 csc 编译，无第三方依赖）。

### 联网说明（全部的网络行为就这一条）

Open365 平时**一个网络请求都不发**。唯一的例外是你**主动**点「检查更新」或敲 `open365 update`：

| | 做什么 | 不做什么 |
|---|---|---|
| 请求 | 向 GitHub 发一个 **空请求体的 GET**，问最新 Release 的版本号 | 不发机器码 / 硬件信息 / 使用统计 / 任何本机数据 |
| 结果 | 告诉你有没有新版、去哪下 | **不后台下载、不静默替换文件、不装任何东西** |
| 时机 | 只在你点了/敲了之后 | **没有开机自动检查，没有后台定时联网** |

实现全在 `engine/update.ps1`（一百多行明文 PowerShell，记事本就能看完）。
不想让它碰 github：`-Url` 或环境变量 `OPEN365_UPDATE_URL` 指到你自己的地址，
指定之后**只查你给的那个源**，不会再偷偷回落到 github。
...
造假残留再清理并验证可还原、造假搬家记录再验证搬家/还原），全程不碰你的真实数据，结束自动还原。
当前 9 个测试套件、100+ 项断言全过。
...
- [x] 软件搬家引擎（微信/QQ/钉钉/企业微信数据目录迁盘；junction+robocopy、先复制成功才删源、可一键还原；47 项离线自测全过）
- [x] 软件搬家图形化（管理中心「软件搬家」页：目标盘输入 + 列表 + 行内搬家/还原；托盘菜单直达）
- [ ] 清理规则扩充（更多浏览器 / 常见软件缓存）
